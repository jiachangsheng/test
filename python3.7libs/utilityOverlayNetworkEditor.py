...
fs_watcher.addPath(os.path.join(currentdir, "utilityOverlayNetworkEditor.py"))
...
class ViewportOutlineWidget(QtWidgets.QWidget):
    ...
    def update(self):
        ...
        s = viewport.qtWindow().size()
        p = viewport.qtWindow().mapToGlobal(QtCore.QPoint(0, 0))
        w = s.width()
        h = s.height()
...
def updateUIElements():
    ...
    if hou.session.lastStatusMessageTimeInMs == -1:
        ...
        if statusMessage.startswith("Hold down Ctrl to snap to rounded values") or \
                ...
                statusMessage.startswith("Successfully saved") or \
                statusMessage.startswith("Undid:") or \
                statusMessage.startswith("Redid:") or \
...
def setOverlayNetworkEditorVisible(visible):
    ...
    if overlayNE:
        if visible:
            overlayNE.show()
            overlayNE.setWindowIconText("visible")
        else:
            overlayNE.hide()
            overlayNE.setWindowIconText("hidden")
...
def toggleOverlayNetworkEditor(ignoreIntegratedNetworkEditor=False):
    ...
    if shiboken2.isValid(overlayNE):
        if overlayNE:
            ...
            if overlayNE.windowIconText() == "visible":
                overlayNE.setWindowIconText("hidden")
                overlayNE.hide()
            else:
                overlayNE.setWindowIconText("visible")
