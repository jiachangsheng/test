...
from utilityOverlayNetworkEditor import toggleOverlayNetworkEditor
...
def getUnshiftedKey(shiftedKey, modifierstate):
    ...
    if modifierstate.shift:
        if (shiftKey not in key):
            shiftedValues = ['~', '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '_', '+', '{', '}', ':', '"', '<',
                             '>', '?', '|', 'Insert', 'Del', 'End', 'DownArrow', 'PageDown', 'LeftArrow', 'Clear',
                             'RightArrow', 'Home', 'UpArrow', 'PageUp']
            unshiftedValues = ["`", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", "[", "]", ";", "'", ",",
                               ".", "/", "\\", "Pad0", "PadDot", "Pad1", "Pad2", "Pad3", "Pad4", "Pad5", "Pad6", "Pad7",
                               "Pad8", "Pad9"]
    ...
    if editortype == hou.paneTabType.NetworkEditor:
        ...
        if nearestNode:
            if nearestNode.isSelected():
                ...
                with hou.undos.group("Select Nearest Node"):
                    nearestNode.setSelected(True, clear_all_selected=True)
    elif editortype == hou.paneTabType.SceneViewer:
        # editor.selectedViewport().frameSelected()
        toggleOverlayNetworkEditor()
    else:
        utility_ui.togglePanel("animatrix_details_view", hou.paneTabType.DetailsView, 0.1)
...
def displaySelectNearestNode(uievent, context):
    with hou.undos.group("Display & Select Nearest Node"):
        ...
        if nearestNode:
            nearestNode.setSelected(True, clear_all_selected=True)
            if context != "Driver" and context != "Shop" and context != "Vop":
...
def randomConstantColor(uievent):
    with hou.undos.group("Random Constant Color"):
        ...
        if newNode:
            ...
            colors = (
                (1, 0, 0), (0, 1, 0), (0, 0, 1), (1, 0.35, 0), (1, 1, 0), (0, 1, 1), (1, 0, 1), (0.45, 0, 1),
                (0.45, 1, 0),
                (0, 0.45, 1))
...
def toFloatOrVector(uievent):
    ...
    if selectedNodes:
        ...
        if outputDataTypes:
            ...
            nodetypes = ['inttofloat', 'floattovec', 'vec2tofloat', 'vectofloat', 'hvectofloat', 'matx2tofloat',
                         'matxtofloat', 'hmatxtofloat']
...
def pointLineProjection(pt, linept0, linept1):
    ...
    plen = direction.normalized().dot(pp.normalized())
    return linept0 + direction * plen * (pp.length() / direction.length())


def dropNodeOnWire(newNode, nodeConnections, pos, editor):
    newNodeType = newNode.type()
    ninputs = newNodeType.maxNumInputs()
    noutputs = newNodeType.maxNumOutputs()

    # print ("nodeConnections", nodeConnections)

    if (len(nodeConnections) == ninputs and len(nodeConnections) == noutputs):
        ...
        sortedwires = sorted(nodeConnections, key=lambda c: pointLineProjection(pos, c.inputNode().position(),
                                                                                c.outputNode().position()).distanceTo(
            pos))

    # print ("sortedwires", sortedwires)
    # sortedwires = sorted(sortedwires, key=lambda c: c.inputIndex())
    # print ("sortedwires2", sortedwires)
    uniquewires = []
    uniquenames = []

    nearestwires = []
    for c in sortedwires:
        dist = pointLineProjection(pos, c.inputNode().position(), c.outputNode().position()).distanceTo(pos)
    ...
    # print ("loop", min(ninputs, noutputs, nwires))
    # print ("nwires", nwires)
    # print ("uniquewires", uniquewires)
    # print "-----------------------------------------"
    ...
    if pwd and path and context:
        ...
        if nodetype:
            ...
            with hou.undos.group("Create New Node: " + nodetype.description()):
                ...
                # print ("size", size)
                pos[0] -= size.x()
                pos[1] -= size.y() + uievent.editor.lengthFromScreen(73) / 2

                setdisplayflag = False
                if nodewires and len(selectedNodes) <= 1:
