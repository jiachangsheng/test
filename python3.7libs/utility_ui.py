...
def getWidgetByName(name):
    ...
    if not hasHandle or (
            hasHandle and getattr(hou.session, name) and not shiboken2.isValid(getattr(hou.session, name))):
...
def resetNetworkEditorZoomLevelFromMousePos(editor=None, targetzoom=125):
    ...
    if editor:
        ...
        bounds = editor.visibleBounds()
        editor.screenBounds().size().x() / bounds.size().x()
...
def appendToParameterEditor():
    """追加到参数编辑器"""
    with hou.undos.disabler():
        nodes = hou.selectedNodes()
        if len(nodes) == 1:
            paneindex = 0
            name = "animatrix_parameter_editor_pinned"
            for i in range(1, 5):
                panename = name + str(i)
                editor = hou.ui.findPaneTab(panename)
                if editor != None:
                    paneindex = i

            if paneindex != 0:
                name = name + str(paneindex)

            print("paneindex", paneindex)
            if paneindex == 0:
                name = "animatrix_parameter_editor"

            if paneindex != 4:
                print("name", name)
                parmEditor = hou.ui.findPaneTab(name)
                newpane = parmEditor.pane().splitVertically()
                newpane.setShowPaneTabs(False)
                panetab = newpane.tabs()[0]
                panetab.setPin(False)
                panetab.setPin(True)
                panetab.setShowNetworkControls(False)
                panetab.setName("animatrix_parameter_editor_pinned" + str(1 + paneindex))

                for i in range(2):
                    fraction = 1 - 1.0 / (2 + paneindex)
                    parmEditor = hou.ui.findPaneTab("animatrix_parameter_editor")
                    parmEditor.pane().setSplitFraction(fraction)
                    if i == 0:
                        print("frac", fraction, "animatrix_parameter_editor")

                    name = "animatrix_parameter_editor_pinned"
                    for index in range(1, 1 + paneindex):
                        panename = name + str(index)
                        panetab = hou.ui.findPaneTab(panename)
                        if panetab != None:
                            fraction = 1 - 1.0 / (2 + paneindex - index)
                            if i == 0:
                                print("frac2", fraction, panename)
                            panetab.pane().setSplitFraction(fraction)


def clearParameterEditor():
    with hou.undos.disabler():
        for i in range(1, 5):
            name = "animatrix_parameter_editor_pinned"
            panename = name + str(i)
...
def currentViewportSwitchToPerspective():
    with hou.undos.disabler():
        ...
        if viewport.isCurrentTab():
            ...
            if view.type() == hou.geometryViewportType.Perspective:
...
def currentViewportToggleViewState():
    ...
    with hou.undos.disabler():
        ...
        viewport = desktop.paneTabOfType(hou.paneTabType.SceneViewer)
        if viewport.isCurrentTab():
            viewstates = ["sopview", "objview", "chopview", "dopview", "imgview", "shopview", "topview", "ropview",
                          "matview"]
